                        ...
                        <>
                            ...
                            <div className="flex items-center gap-2 bg-card p-1.5 rounded-[1.5rem] border border-border overflow-x-auto no-scrollbar shadow-inner">
                                {[
                                    ...
                                    ...(dbUser.role === 'superadmin' ? [{ id: "superadmin", label: "ADMIN", icon: ShieldCheck }] : []),
                                    ...
                                    <div className="max-w-2xl mx-auto">
                                        <div className="bg-card border border-border rounded-[2rem] p-8 md:p-12 shadow-2xl">
                                            <div className="flex items-center gap-3 mb-6">
                                                ...
                                                <h2 className="text-2xl font-black uppercase italic tracking-tight">Invitaciones de Club</h2>
                                            </div>

                                            <p className="text-muted-foreground text-xs font-medium mb-8 leading-relaxed uppercase tracking-widest">
                                                Envía un correo de invitación para que un nuevo usuario se registre con el rol de <span className="text-indigo-600 dark:text-indigo-400 font-black">CLUB / CENTRO</span>. Al aceptar la invitación, el rol se asignará automáticamente.
                                            </p>
                                            ...
                                            <div className="flex flex-col gap-6">
                                                <div className="bg-card border border-border rounded-3xl p-6 relative overflow-hidden group">
                                                    ...
                                                    <h3 className="text-[10px] font-black uppercase text-muted-foreground mb-4 flex items-center gap-2">
                                                        <Activity className="h-3 w-3 text-emerald-500" /> Link para WhatsApp
                                                    </h3>
                                                    <div className="flex items-center gap-3">
                                                        <div className="flex-1 bg-muted/50 border border-border rounded-xl px-4 py-3 text-[10px] text-foreground font-mono truncate select-all shadow-inner">
                                                            {typeof window !== 'undefined' ? `${window.location.origin}/onboarding?role=club` : '...'}
                                                        </div>
                                                        <button
                                                            onClick={() => {
                                                                const link = `${window.location.origin}/onboarding?role=club`;
                                                                navigator.clipboard.writeText(link);
                                                                toast.success("Link copiado para WhatsApp");
                                                            }}
                                                            className="px-4 py-3 bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-400 rounded-xl transition-all flex items-center gap-2 text-[10px] font-black uppercase tracking-widest"
                                                        >
                                                            Copiar
                                                        </button>
                                                    </div>
                                                    <p className="mt-4 text-[9px] text-muted-foreground/60 font-medium leading-relaxed italic">
                                                        * Este link redirige al onboarding del usuario con el rol de <span className="text-muted-foreground">Club</span> pre-seleccionado.
                                                    </p>
                                                </div>

                                                <div className="flex items-center gap-4 py-2">
                                                    <div className="h-px flex-1 bg-card" />
                                                    <span className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-[0.3em]">o vía email</span>
                                                    <div className="h-px flex-1 bg-card" />
                                                </div>
                                                ...
                                                >
                                                    <div className="flex flex-col gap-2">
                                                        <label className="text-[10px] font-black uppercase text-muted-foreground ml-2">Correo del Club</label>
                                                        <div className="relative group">
                                                            <input
                                                                type="email"
                                                                name="email"
                                                                placeholder="club-ejemplo@acap.com"
                                                                required
                                                                className="w-full bg-card border border-border rounded-2xl py-4 px-6 text-foreground text-sm font-bold outline-none focus:border-indigo-500 transition-all shadow-inner group-hover:bg-muted/50"
                                                            />
                                                        </div>
                                                    </div>
                                                    ...
                                                    <button
                                                        type="submit"
                                                        className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-900/40 flex items-center justify-center gap-2"
                                                    >
                        ...
                        <div className="fixed inset-0 bg-black/90 backdrop-blur-2xl z-[1000] flex items-center justify-center p-4">
                            <div className="bg-card border border-border rounded-[2.5rem] w-full max-w-[500px] max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 shadow-2xl">
                                ...
                                <form onSubmit={handleSave} className="p-8 flex flex-col gap-6">
                                    <div className="flex flex-col gap-2">
                                        <label className="text-[10px] font-black uppercase text-muted-foreground ml-2">Nombre en Pista</label>
                                        <input
                                            type="text"
                                            value={formData.name}
                                            onChange={e => setFormData({ ...formData, name: e.target.value })}
                                            className="w-full bg-card border border-border rounded-2xl py-4 px-5 text-white text-sm font-bold outline-none focus:border-indigo-500 shadow-inner"
                                        />
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="flex flex-col gap-2">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <label className="text-[10px] font-black uppercase text-muted-foreground ml-2">Lado</label>
                                            <select
                                                value={formData.side}
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <label className="text-[10px] font-black uppercase text-muted-foreground ml-2">BIO</label>
                                        <textarea
                                            value={formData.bio}
                                            onChange={e => setFormData({ ...formData, bio: e.target.value })}
                                            rows={4}
                                            className="w-full bg-card border border-border rounded-2xl py-4 px-5 text-white text-sm font-bold outline-none focus:border-indigo-500 resize-none shadow-inner"
                                        />
                                    </div>
                                    <div className="grid grid-cols-2 gap-4 mt-4">
                                        <button type="button" onClick={() => setIsEditing(false)} className="bg-card text-muted-foreground border border-border py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">Cancelar</button>
