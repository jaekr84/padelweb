...
const selectTrigger =
    "flex items-center gap-1.5 h-8 px-3 rounded-lg border border-hairline bg-surface text-muted-foreground text-[9px] font-black uppercase tracking-widest outline-none hover:border-volt/40 hover:text-volt-ink transition-all min-w-[110px] max-w-[160px]";
const selectContent =
    "z-50 overflow-hidden rounded-xl border border-hairline bg-background shadow-2xl shadow-black/50 min-w-[var(--radix-select-trigger-width)] animate-in fade-in zoom-in-95 duration-150";
const selectItem =
    "relative flex cursor-default select-none items-center rounded-lg pl-3 pr-8 py-2 text-[10px] font-bold text-muted-foreground outline-none transition-colors data-[highlighted]:bg-surface-raised data-[highlighted]:text-foreground data-[state=checked]:bg-volt/15 data-[state=checked]:text-volt-ink";
...
export default function TournamentFiltersClient({
    ...
    const getTabStyle = (value: string) => {
        ...
        if (!isActive) return "bg-surface text-muted-foreground border-hairline hover:border-hairline-strong hover:text-foreground";
            ...
            <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Select.Content className={selectContent} position="popper" sideOffset={6}>
                            <Select.Viewport className="p-1">
                                <Select.Item value="todas" className={selectItem}>
                                    ...
                                    <Select.ItemIndicator className="absolute right-3 text-volt-ink">
                ...
                {hasActiveFilters && (
                    <button
                        type="button"
                        onClick={clearAll}
                        className="flex items-center gap-1 h-8 px-2.5 rounded-lg border border-hairline bg-transparent text-muted-foreground text-[9px] font-black uppercase tracking-widest hover:border-live/60 hover:text-live hover:bg-live/5 transition-all"
                    >
                        <X className="w-3 h-3" />
                        Limpiar
                    </button>
