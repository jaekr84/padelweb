...
export function AmericanoHeader({
    ...
    return (
        <header className="sticky top-0 z-[60] bg-background/60 backdrop-blur-3xl border-b border-border/40">
            <div className="w-full px-3 md:px-4 h-11 flex items-center justify-between gap-3">
                <div className="flex items-center gap-4">
                    <button
                        ...
                        className="group flex items-center gap-2 text-foreground/70 hover:text-foreground transition-all px-3 py-1.5 hover:bg-muted rounded-lg"
                    >
                        <ArrowLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
                        <span className="text-[9px] font-black uppercase tracking-[0.15em] italic">Volver</span>
                    </button>

                    <div className="h-6 w-px bg-border/30 hidden md:block" />

                    <div className="hidden md:flex flex-col min-w-0">
                        <span className="text-[7px] font-black uppercase tracking-[0.2em] text-azul-primary leading-none mb-0.5">Torneo</span>
                        <span className="text-[10px] font-black uppercase italic tracking-tight text-foreground/90 leading-none truncate max-w-[120px] lg:max-w-[200px]">
                            {tournamentName}
                        </span>
                    </div>
                                    ...
                                    <div key={s.id} className="flex items-center">
                                        <button
                                            ...
                                            className={`flex items-center gap-1.5 px-3 py-1 rounded-lg transition-all ${s.active
                                                ? "bg-azul-primary text-white shadow-sm shadow-azul-primary/10"
                                                : s.completed
                                        ...
                                        >
                                            ...
                                            {s.completed && <Check className="w-2.5 h-2.5 lg:w-3 lg:h-3 ml-0.5" />}
                                        </button>
                                        {idx < 1 && <ChevronRight className="w-3 h-3 lg:w-3.5 lg:h-3.5 mx-0.5 text-border/30" />}
                                    </div>
                                );
                            });
                        })()}
                ...
                <div className="flex items-center gap-3">
                    <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-azul-primary/5 border border-azul-primary/20 text-azul-primary text-[9px] font-black uppercase tracking-widest">
