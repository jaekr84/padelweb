...
export default function OpenCourtPublicClient({ initialEvents, userRegistrations, isLoggedIn, currentUserId, userRole }: OpenCourtPublicClientProps) {
    ...
    const dropdownTriggerStyles = "w-full rounded-2xl border border-hairline bg-background py-2.5 pl-10 pr-4 text-[11px] font-bold uppercase tracking-widest text-foreground focus:outline-none focus:ring-1 focus:ring-azul-primary/30 transition-all flex items-center justify-between gap-2 shadow-sm shadow-azul-primary/5";
    const dropdownContentStyles = "z-50 overflow-hidden rounded-xl border border-hairline bg-background shadow-2xl animate-in fade-in zoom-in-95 duration-200";
    const dropdownViewportStyles = "p-1";
    const dropdownItemStyles = "relative flex cursor-default select-none items-center rounded-lg px-4 py-2 text-[10px] font-black tracking-widest uppercase text-foreground outline-none transition-colors data-[highlighted]:bg-azul-primary/10 data-[highlighted]:text-azul-primary data-[state=checked]:bg-azul-primary data-[state=checked]:text-foreground";
    ...
    return (
        <div className="relative min-h-screen bg-grid-carbon text-foreground font-sans selection:bg-volt/30">
            {/* Ambient glow */}
            ...
            <div className="relative z-10 max-w-[1600px] mx-auto px-4 py-4 md:py-6 space-y-5">
                ...
                <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            <span className="label-tech text-[9px] text-volt-ink">ACAP</span>
                            <div className="h-px w-6 bg-volt/30" />
                        </div>
                        <div>
                            <h1 className="heading-sport text-2xl md:text-3xl leading-none text-foreground">
                                Cancha <span className="text-celeste-light drop-shadow-[0_0_15px_rgba(56,189,248,0.25)]">Abierta</span>
                            </h1>
                            <p className="label-tech text-[9px] text-subtle mt-0.5">
                    ...
                    {userRole === "club" && (
                        <Link href="/admin/cancha-abierta/create">
                            <button className="group clip-notch bg-rojo hover:bg-rojo-dark text-white pl-4 pr-3 py-2 font-black text-[9px] uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-rojo/30 flex items-center gap-3 shrink-0">
                                Crear Evento
                                <div className="w-6 h-6 rounded-lg bg-surface-raised flex items-center justify-center group-hover:bg-surface-raised transition-colors">
                ...
                <div className="relative bg-background border border-hairline rounded-xl overflow-hidden shadow-sm">
                    <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-hairline pointer-events-none" />
                    <div className="absolute top-0 right-0 w-2 h-2 border-t border-r border-hairline pointer-events-none" />
                    <div className="absolute bottom-0 left-0 w-2 h-2 border-b border-l border-hairline pointer-events-none" />
                    <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-hairline pointer-events-none" />
                    <div className="flex items-center divide-x divide-hairline overflow-x-auto">
                        {[
                            ...
                            { icon: Trophy, color: "text-subtle", value: pastEvents.length, label: "Finalizados" },
                            ...
                            <div key={i} className="flex items-center gap-2 px-4 py-2 shrink-0 group hover:bg-surface transition-colors">
                                <kpi.icon className={`w-3 h-3 shrink-0 ${kpi.color}`} />
                                <span className="text-[11px] font-black font-mono text-foreground leading-none">{kpi.value}</span>
                                <span className="text-[8px] font-black uppercase tracking-widest text-subtle leading-none">{kpi.label}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="relative bg-background border border-hairline rounded-xl overflow-hidden shadow-sm flex items-center divide-x divide-hairline">
                    <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-hairline pointer-events-none z-10" />
                    <div className="absolute top-0 right-0 w-2 h-2 border-t border-r border-hairline pointer-events-none z-10" />
                    <div className="absolute bottom-0 left-0 w-2 h-2 border-b border-l border-hairline pointer-events-none z-10" />
                    <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-hairline pointer-events-none z-10" />

                    {/* Search Input */}
                    <div className="relative flex-1 min-w-0">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3 h-3 text-subtle pointer-events-none" />
                        <input
                            type="text"
                            value={textQuery}
                            onChange={(e) => setTextQuery(e.target.value)}
                            placeholder="Buscar cancha o club..."
                            className="w-full bg-transparent py-2 pl-8 pr-3 text-[9px] font-bold uppercase tracking-widest text-foreground focus:outline-none placeholder:text-subtle h-8 transition-all"
                        />
                    </div>

                    {/* City Select */}
                    <Select.Root value={cityQuery} onValueChange={(value) => setCityQuery(value === "__empty__" ? "" : value)}>
                        <Select.Trigger className="flex items-center gap-1.5 px-3 h-8 text-[9px] font-bold uppercase tracking-widest text-foreground focus:outline-none transition-all hover:bg-surface shrink-0 bg-transparent w-36">
                            <MapPin className="w-3 h-3 text-celeste shrink-0" />
                            <Select.Value placeholder="Ciudad..." />
                            <Select.Icon className="ml-auto">
                                <ChevronRight className="w-2.5 h-2.5 text-subtle rotate-90" />
                            </Select.Icon>
                        </Select.Trigger>
                        <Select.Portal>
                    ...
                    <Select.Root value={sortBy} onValueChange={(value) => setSortBy(value)}>
                        <Select.Trigger className="flex items-center gap-1.5 px-3 h-8 text-[9px] font-bold uppercase tracking-widest text-foreground focus:outline-none transition-all hover:bg-surface shrink-0 bg-transparent w-40">
                    ...
                    <div className="flex items-center shrink-0">
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 h-8 text-[9px] font-black uppercase tracking-widest transition-all ${viewMode === "grid" ? "bg-azul-primary/10 text-azul-primary" : "text-subtle hover:text-foreground hover:bg-surface"
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 h-8 text-[9px] font-black uppercase tracking-widest transition-all ${viewMode === "table" ? "bg-orange-500/10 text-orange-500" : "text-subtle hover:text-foreground hover:bg-surface"
                                }`}
                        >
                    ...
                    {viewMode === "grid" ? (
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
                            ...
                            {upcomingEvents.length === 0 && (
                                <div className="col-span-full py-16 flex flex-col items-center justify-center space-y-4 bg-surface border border-dashed border-hairline rounded-[2.5rem]">
                                    <Search className="w-10 h-10 text-muted-foreground/10" />
                                    <div className="text-center">
                                        <h4 className="text-sm font-black uppercase tracking-tighter italic text-subtle">Sin eventos disponibles</h4>
                        ...
                        <div className="bg-background border border-hairline rounded-2xl overflow-hidden shadow-xl animate-in fade-in duration-300">
                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse min-w-[600px]">
                                    <thead>
                                        <tr className="border-b border-hairline bg-surface text-[8px] font-black tracking-widest uppercase text-muted-foreground/80 h-9">
                                    ...
                                    <tbody>
                                        {upcomingEvents.map((event) => {
                                            ...
                                            return (
                                                <tr key={event.id} className="border-b border-hairline hover:bg-surface transition-colors h-14 text-xs">
                                                    {/* Club / Name */}
                                                    <td className="px-4 py-2 min-w-[200px]">
                                                        <div className="flex items-center gap-2.5">
                                                            ...
                                                            <div className="min-w-0">
                                                                <h4 className="text-[11px] font-black uppercase italic truncate tracking-tight text-foreground">
                                                                    {event.name}
                                                                </h4>
                                                                <p className="text-[8px] font-black uppercase tracking-wider text-subtle truncate">
                                                                    {event.club.name}
                                                                </p>
                                                            </div>
                                                        </div>
                                                    ...
                                                    {/* Date / Time */}
                                                    <td className="px-4 py-2 font-mono text-[10px] text-foreground/85">
                                                        <div className="flex flex-col">
                                                            <span className="font-bold flex items-center gap-1 text-[10px]">
                                                                <Calendar className="w-3 h-3 text-celeste shrink-0" />
                                                                {formatDate(event.date)}
                                                            </span>
                                                            <span className="text-muted-foreground flex items-center gap-1 text-[9px] mt-0.5">
                                                    ...
                                                    <td className="px-4 py-2">
                                                        <div className="flex flex-col gap-1 w-28">
                                                            <div className="flex items-center justify-between text-[9px] font-black font-mono">
                                                                <span className="text-muted-foreground">{event.registrationCount} / {event.totalSlots || 0}</span>
                                                                <span className="text-subtle">{Math.round(percent)}%</span>
                                                            </div>
                                                            <div className="h-1 w-full bg-surface-raised border border-hairline rounded-full overflow-hidden">
                                                                <div
                                                    ...
                                                    <td className="px-4 py-2 text-right">
                                                        <div className="flex items-center justify-end gap-1.5">
                                                            ...
                                                            <button
                                                                ...
                                                                className="h-7 px-2 bg-background hover:bg-surface-raised border border-hairline text-muted-foreground hover:text-foreground rounded text-[8px] font-black uppercase tracking-widest transition-all flex items-center gap-1"
                                                            ...
                                                            {isLoggedIn && (event.creatorId || event.club?.ownerId) && currentUserId !== (event.creatorId || event.club?.ownerId) && (
                                                                <button
                                                                    ...
                                                                    className="h-7 w-7 bg-background hover:bg-surface-raised border border-hairline text-muted-foreground hover:text-azul-primary rounded flex items-center justify-center transition-all"
                                        ...
                                        {upcomingEvents.length === 0 && (
                                            <tr>
                                                <td colSpan={5} className="py-12 text-center">
                                                    ...
                                                    <h4 className="text-xs font-black uppercase tracking-tighter italic text-subtle">Sin eventos disponibles</h4>
                                                    <p className="text-[8px] font-bold text-muted-foreground/30 uppercase tracking-widest">Intentá con otra ciudad o filtro</p>
                                                </td>
                                            </tr>
                                        )}
                                    </tbody>
                    ...
                    <div className="pt-12 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
                        <div className="flex items-center gap-4 px-2">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-subtle whitespace-nowrap">Resultados Recientes</h2>
                            </div>
                            <div className="h-px flex-1 bg-border" />
                        </div>
                        ...
                        {viewMode === "grid" ? (
                            ...
                            <div className="bg-background border border-hairline rounded-2xl overflow-hidden shadow-xl opacity-90">
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left border-collapse min-w-[600px]">
                                        <thead>
                                            <tr className="border-b border-hairline bg-surface text-[8px] font-black tracking-widest uppercase text-muted-foreground/80 h-9">
                                            ...
                                            {pastEvents.map((event) => {
                                                ...
                                                return (
                                                    <tr key={event.id} className="border-b border-hairline hover:bg-surface transition-colors h-14 text-xs">
                                                        <td className="px-4 py-2">
                                                            <div className="flex items-center gap-2.5">
                                                                <div className="w-7 h-7 rounded-lg bg-surface-raised border border-hairline flex items-center justify-center shrink-0 text-[10px] font-black uppercase text-muted-foreground overflow-hidden">
                                                                    {event.club.image ? (
                                                                        <img src={event.club.image} alt={event.club.name || ""} className="w-full h-full object-cover" />
                                                                    ) : (
                                                                        (event.club.name || "C").charAt(0)
                                                                ...
                                                                <div>
                                                                    <h4 className="text-[11px] font-black uppercase italic truncate text-muted-foreground line-through decoration-muted-foreground/40">
                                                                        {event.name}
                                                                    </h4>
                                                                    <p className="text-[8px] font-black uppercase tracking-wider text-muted-foreground/45 truncate">
                                                                        {event.club.name}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-2 font-mono text-[10px] text-subtle">
                                                            <div className="flex flex-col">
                                                                <span className="flex items-center gap-1 text-[10px]">
                                                                    <Calendar className="w-3 h-3 shrink-0 text-subtle" />
                                                                    {formatDate(event.date)}
                                                                </span>
                                                                <span className="flex items-center gap-1 text-[9px] mt-0.5">
                                                                    <Clock className="w-3 h-3 shrink-0 text-subtle" />
                                                                    {event.time} HS
                                                                </span>
                                                            </div>
                                                        </td>
                                                        ...
                                                        <td className="px-4 py-2">
                                                            <span className="inline-flex items-center px-1.5 py-0.5 rounded border border-hairline text-muted-foreground bg-surface text-[7px] font-black uppercase tracking-widest">
                                                                Finalizado
                                                            </span>
                                                        </td>
                                                        <td className="px-4 py-2 text-right">
            ...
            <div className="max-w-[1600px] mx-auto px-4 pb-12">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-6">
                    {[
                        ...
                        <div key={i} className="group relative bg-background hover:bg-azul-primary/[0.02] border border-hairline rounded-xl p-4 flex items-center gap-3 transition-all duration-300 shadow-sm hover:shadow-md hover:border-azul-primary/20">
                            <div className="absolute top-0 left-0 w-1.5 h-1.5 border-t border-l border-hairline pointer-events-none" />
                            <div className="absolute bottom-0 right-0 w-1.5 h-1.5 border-b border-r border-hairline pointer-events-none" />
                            <div className={`w-8 h-8 rounded-lg bg-surface-raised flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform ${item.color}`}>
                                <item.icon className="w-4 h-4" />
                            </div>
                            <div className="space-y-0.5 min-w-0">
                                <h4 className="text-[10px] font-black uppercase tracking-[0.15em] text-foreground">{item.title}</h4>
                                <p className="text-[9px] font-bold text-muted-foreground leading-snug italic">{item.desc}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {isParticipantsModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <motion.div
                            ...
                            className="relative w-full max-w-2xl bg-background border border-hairline rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] z-10"
                        >
                            ...
                            {/* Header */}
                            <div className="p-4 border-b border-hairline flex items-center justify-between bg-surface shrink-0">
                                <div>
                                    <span className="text-[7px] font-black uppercase tracking-[0.3em] text-celeste">LISTADO DE ASISTENCIA</span>
                                    <h3 className="text-sm font-black uppercase tracking-tight italic text-foreground truncate max-w-md mt-0.5">
                                        {selectedEventName}
                                    </h3>
                                </div>
                                <button
                                    onClick={() => setIsParticipantsModalOpen(false)}
                                    className="h-8 w-8 rounded-lg bg-surface-raised hover:bg-surface-raised text-muted-foreground hover:text-foreground flex items-center justify-center transition-all border border-hairline"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                            ...
                            <div className="p-3 bg-surface border-b border-hairline shrink-0">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full rounded-lg border border-hairline bg-background py-2 pl-9 pr-4 text-[10px] font-bold uppercase tracking-widest text-foreground focus:outline-none focus:ring-1 focus:ring-azul-primary/30 placeholder-muted-foreground/50 h-8.5 transition-all"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 min-h-[250px]">
                                {isLoadingRegistrations ? (
                                    <div className="h-full flex flex-col items-center justify-center py-16 space-y-3">
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-widest text-subtle">Cargando inscriptos...</span>
                                    </div>
                                    ...
                                    <div className="py-16 flex flex-col items-center justify-center space-y-3 bg-surface border border-dashed border-hairline rounded-xl">
                                        <Search className="w-8 h-8 text-muted-foreground/20" />
                                        <div className="text-center">
                                            <h4 className="text-xs font-black uppercase tracking-tight italic text-subtle">Sin inscriptos</h4>
                                            <p className="text-[8px] font-bold text-muted-foreground/45 uppercase tracking-widest mt-0.5">Ningún inscripto coincide con la búsqueda</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            {/* Footer */}
                            <div className="p-3 border-t border-hairline bg-surface text-right shrink-0 flex items-center justify-between">
                                <span className="text-[8px] font-black uppercase tracking-widest text-subtle font-mono">
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[250px]">
                                {isLoadingResults ? (
                                    <div className="h-full flex flex-col items-center justify-center py-16 space-y-3">
                                        <div className="w-6 h-6 border-2 border-celeste border-t-transparent rounded-full animate-spin" />
                                        <span className="text-[9px] font-black uppercase tracking-widest text-subtle">Cargando reporte de partidos...</span>
                                    </div>
                                    ...
                                    <>
                                                            ...
                                                            <div key={player.userId || "inv"} className="flex items-center justify-between mt-2">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <p className="text-[7px] font-bold text-muted-foreground uppercase mt-0.5 tracking-wider">Monitoreo de rendimiento</p>
                                                                    </div>
                                                                </div>
                                                                <div className="flex items-center gap-4">
                                                                    <div className="text-center">
                                                                        <span className="text-[8px] font-black text-muted-foreground uppercase block leading-none">PARTIDOS</span>
                                                                        <span className="text-[11px] font-black font-mono text-foreground mt-0.5 block">{stat.played}</span>
                                                                    </div>
                                                                    <div className="text-center">
                                                                        <span className="text-[8px] font-black text-muted-foreground uppercase block leading-none">VICTORIAS</span>
                                                                        <span className="text-[11px] font-black font-mono text-emerald-400 mt-0.5 block">{stat.won}</span>
                                                                    </div>
                                                                    <div className="text-center">
                                                                        <span className="text-[8px] font-black text-muted-foreground uppercase block leading-none">EFICACIA</span>
                                                                        <span className="text-[11px] font-black font-mono text-celeste mt-0.5 block">{winRate}%</span>
                                                                    </div>
                                                                    <div className="text-center">
                                                                        <span className="text-[8px] font-black text-muted-foreground uppercase block leading-none">PUNTOS</span>
                                        ...
                                        <div className="grid grid-cols-1 gap-2.5">
                                            {filteredMatches.map((match) => (
                                                <div
                                                    key={match.id}
                                                    className="bg-background/70 border border-hairline p-3.5 rounded-xl flex flex-col gap-2.5 relative overflow-hidden transition-all duration-300 hover:border-celeste/35"
                                                    style={{
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center justify-between border-b border-hairline pb-1.5">
                                                        <span className="text-[7px] font-black uppercase tracking-[0.25em] text-subtle flex items-center gap-1">
                                                            <Zap className="w-2.5 h-2.5 text-orange-500" />
                                                            Cancha {match.courtNumber || 1}
                                                        </span>
                                                        {match.finishedAt && (
                                                            <span className="text-[7px] font-bold font-mono text-subtle">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <div className="flex flex-col items-center shrink-0 gap-0.5">
                                                            <div
                                                                className="flex items-center justify-center gap-2 bg-background border border-slate-700/60 px-3 py-1 text-xs font-black font-mono skew-x-[-12deg] shadow-inner text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.3)] min-w-[60px] h-7"
                                                            >
                                    ...
                                    <div className="py-16 flex flex-col items-center justify-center space-y-3 bg-surface border border-dashed border-hairline rounded-xl">
                                        ...
                                        <div className="text-center">
                                            <h4 className="text-xs font-black uppercase tracking-tight italic text-subtle">Sin partidos registrados</h4>
                                            <p className="text-[8px] font-bold text-muted-foreground/45 uppercase tracking-widest mt-0.5">No hay partidos finalizados cargados en este evento</p>
                                        </div>
                                    </div>
                                )}
                            </div>
            ...
            >
                {image ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-background">
                        <span className="text-xl font-black text-subtle uppercase">{name[0]}</span>
                    </div>
                )}
                {/* Gradient overlay */}
                ...
                {/* Side badge */}
                {side && (
                    <div className="absolute top-1 right-1 px-1 py-0.5 bg-black/70 border border-hairline rounded text-[6px] font-black uppercase tracking-wider text-muted-foreground">
                        {side}
                    </div>
                )}
                {/* Winner star */}
                {isWinner && (
                    <div className="absolute top-1 left-1 w-3 h-3 rounded-full bg-emerald-500/90 flex items-center justify-center">
                        <span className="text-[7px] text-foreground font-black">★</span>
                    </div>
                )}
                {/* Name plate */}
                <div
                ...
                >
                    <p className="text-[7px] font-black uppercase tracking-wider text-foreground truncate leading-none" style={{ transform: "skewX(6deg)" }}>
                        {firstName}
                    </p>
                    {lastName && (
                        <p className="text-[6px] font-bold uppercase tracking-wider text-foreground/70 truncate leading-none mt-0.5" style={{ transform: "skewX(6deg)" }}>
                            {lastName}
                        </p>
                    )}
                </div>
