...
export default function SponsorManagementClient({ initialSponsors }: Props) {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-12 pb-24 font-sans">
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-hairline pb-10">
                <div className="space-y-1">
                    ...
                    <h1 className="text-4xl md:text-5xl heading-sport text-foreground">
            ...
            {isAdding && (
                <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="bg-card border border-hairline rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
                        ...
                        <form onSubmit={handleSaveSponsor} className="space-y-8">
                                ...
                                <div className="space-y-6 text-left">
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-surface border border-hairline rounded-2xl px-5 py-4 text-foreground focus:outline-none focus:ring-1 focus:ring-azul-primary/50 transition-all"
                                            required
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-surface border border-hairline rounded-2xl pl-12 pr-5 py-4 text-foreground focus:outline-none focus:ring-1 focus:ring-azul-primary/50 transition-all"
                                            />
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="space-y-2">
                                            ...
                                            <div className="relative group">
                                                {previewUrl ? (
                                                    <div className="w-full h-48 bg-surface border-2 border-dashed border-azul-primary/30 rounded-2xl overflow-hidden flex items-center justify-center p-8 group-hover:border-azul-primary/60 transition-all">
                                                        <div className="relative w-full h-full">
                                                            <Image src={previewUrl} alt="Preview" fill className="object-contain" />
                                                        </div>
                                                    </div>
                                                ) : (
                                                    <div className="w-full h-48 bg-surface border-2 border-dashed border-hairline rounded-2xl flex flex-col items-center justify-center gap-3 text-muted-foreground group-hover:text-muted-foreground group-hover:border-azul-primary/30 transition-all">
                                        ...
                                        {previewUrl && (
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <div className="w-44 rounded-xl overflow-hidden border border-hairline shadow-xl bg-white shrink-0">
                                                        {/* Celda real del sponsor — mismo ancho (176px) y altura que el sidebar */}
                                                        <div className="relative h-[112px] overflow-hidden border-b border-border bg-white px-3 py-2">
                                                            <img src={previewUrl} alt="Vista previa" className="w-full h-full object-contain" />
                                                        </div>
                                                        {/* Celda vecina (mismo tamaño) para dar contexto */}
                                                        <div className="h-[112px] bg-muted flex items-center justify-center">
                                                            <span className="text-[7px] font-bold uppercase tracking-widest text-muted-foreground">Otro sponsor</span>
                                                        </div>
                                                    </div>
                            ...
                            <div className="flex gap-4 pt-4">
                                ...
                                <button
                                    ...
                                    className="px-8 bg-surface-raised text-foreground font-black uppercase tracking-widest text-[10px] py-4 rounded-2xl border border-hairline hover:bg-rose-900/40 hover:text-rose-400 hover:border-rose-900/50 transition-all"
                                >
            ...
            <div className="space-y-6">
                <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground flex items-center gap-3">
                    <span className="w-8 h-px bg-surface-raised" /> Marcas Registradas
                </h2>

                {sponsors.length === 0 ? (
                    <div className="bg-card/40 border border-hairline rounded-[2rem] p-20 text-center flex flex-col items-center gap-4">
                        <ImageIcon className="w-12 h-12 text-subtle" />
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                        {sponsors.map((sponsor) => (
                            <div key={sponsor.id} className="group relative bg-card border border-hairline rounded-2xl p-6 transition-all hover:border-azul-primary/40 hover:shadow-2xl hover:shadow-azul-primary/5">
