...
export default function AdminOpenCourtClient({ initialEvents }: Props) {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-4 animate-fade-in pb-12">
            {/* Header */}
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/10 pb-3">
                <div>
                    ...
                    <p className="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400 mt-1 max-w-md">
            ...
            {/* Filters */}
            <div className="flex flex-col md:flex-row gap-3 items-center bg-carbon-800/40 backdrop-blur-md p-1.5 rounded-xl border border-white/10">
                <div className="relative flex-1 group">
                    <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 transition-colors group-focus-within:text-azul-primary" />
                    <input
                        type="text"
                        placeholder="Buscar por nombre, dirección o zona..."
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full bg-transparent border-none py-2 pl-10 pr-4 text-xs font-medium focus:outline-none focus:ring-0 placeholder:text-slate-500"
                    />
                </div>
    ...
    return (
        <tr className="hover:bg-white/5 transition-colors group align-middle">
            <td className="px-3 py-2">
                <span className="text-[12px] font-black uppercase italic text-white leading-tight group-hover:text-azul-primary transition-colors truncate block max-w-[220px] capitalize">
                    {event.name}
                </span>
            </td>
            <td className="px-3 py-2">
                <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className="text-[11px] font-bold text-slate-400 truncate capitalize">
            ...
            <td className="px-3 py-2">
                <span className="text-[11px] font-bold text-slate-200 tabular-nums">{event.time}</span>
            </td>
            <td className="px-3 py-2">
                <span className="text-[11px] font-black text-slate-200 tabular-nums">
                    {event.registrations?.length || 0}<span className="text-slate-500 font-bold"> / {event.totalSlots}</span>
                </span>
            </td>
            <td className="px-3 py-2">
            ...
            <td className="px-3 py-2 text-right">
                <div className="flex items-center justify-end gap-1">
                    <Link href={`/admin/cancha-abierta/${event.id}${!isActive ? '?tab=history' : ''}`}>
                        <button className={`h-7 flex items-center justify-center gap-1 px-2.5 rounded-lg transition-all active:scale-95 shadow-sm text-[7px] font-black uppercase tracking-widest ${isActive ? 'bg-azul-primary hover:bg-azul-dark text-white border border-azul-primary/20' : 'bg-white/10 text-slate-400 border border-white/10 hover:bg-white/15'}`}>
                            <Activity className="w-2.5 h-2.5" />
                            {isActive ? 'Gestión' : 'Ver'}
                        </button>
                    </Link>
                    <button className="w-7 h-7 flex items-center justify-center bg-white/10 hover:bg-celeste hover:text-white text-slate-400 border border-white/10 rounded-lg transition-all active:scale-95" title="Editar">
                        <Edit className="w-3 h-3" />
                    </button>
                    <button
                        onClick={() => onDelete(event.id)}
                        className="w-7 h-7 flex items-center justify-center bg-white/10 hover:bg-rojo hover:text-white text-slate-400 border border-white/10 rounded-lg transition-all active:scale-95"
                        title="Eliminar"
                    >
