...
export default function AdminOpenCourtClient({ initialEvents }: Props) {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-4 animate-fade-in pb-12">
            {/* Header */}
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-hairline pb-3">
                <div>
            ...
            {/* Filters */}
            <div className="flex flex-col md:flex-row gap-3 items-center bg-card/40 backdrop-blur-md p-1.5 rounded-xl border border-hairline">
                <div className="relative flex-1 group">
                    ...
                    <input
                        ...
                        className="w-full bg-transparent border-none py-2 pl-10 pr-4 text-xs font-medium focus:outline-none focus:ring-0 placeholder:text-subtle"
                    />
                </div>
        ...
        <tr className="hover:bg-surface transition-colors group align-middle">
            ...
            <td className="px-3 py-2">
                <div className="flex items-center gap-1.5 min-w-0">
                    <MapPin className="w-2.5 h-2.5 text-subtle shrink-0" />
                    <span className="text-[11px] font-bold text-muted-foreground truncate capitalize">
            ...
            <td className="px-3 py-2">
                <span className="text-[11px] font-bold text-muted-foreground tabular-nums">{event.time}</span>
            </td>
            <td className="px-3 py-2">
                <span className="text-[11px] font-black text-muted-foreground tabular-nums">
                    {event.registrations?.length || 0}<span className="text-subtle font-bold"> / {event.totalSlots}</span>
                </span>
            </td>
            <td className="px-3 py-2">
