...
import { Plus, Trash2, Save, MoveUp, MoveDown, Layers } from "lucide-react";
...
export default function CategoriesManager({ initialCategories }: { initialCategories: any[] }) {
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            setNewCat({ name: "", minPoints: 0, maxPoints: 1000, gender: "mixto" });
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="flex gap-2">
                        <input
                            ...
                            onChange={e => setNewCat({ ...newCat, minPoints: parseInt(e.target.value) })}
                        />
                        <input
                            type="number"
                            placeholder="Max"
                            ...
                            onChange={e => setNewCat({ ...newCat, maxPoints: parseInt(e.target.value) })}
                        />
                    </div>
                    <select
                        className="bg-muted border border-border rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-indigo-500 transition-all text-foreground appearance-none"
                        value={newCat.gender}
                ...
                <div className="divide-y divide-border">
                    {categories.map((cat, idx) => (
                        ...
                        >
                            ...
                            {/* Gender - Desktop */}
                            <div className="hidden md:block col-span-2 text-[10px] font-black uppercase tracking-widest">
                                <span className={`px-2 py-1 rounded-md border ${
                                    cat.gender === 'masculino' ? 'bg-blue-500/10 border-blue-500/20 text-blue-500' :
                                    cat.gender === 'femenino' ? 'bg-rose-500/10 border-rose-500/20 text-rose-500' :
