...
export default function RequestsClient() {
    ...
    const getStatusStyle = (status: string, createdAt: Date) => {
        ...
        switch (currentStatus) {
            case 'pendiente': return { bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', text: 'text-indigo-600', icon: Clock, label: 'Pendiente' };
            case 'enviado': return { bg: 'bg-blue-500/10', border: 'border-blue-500/20', text: 'text-blue-600', icon: Send, label: 'Enviado' };
            case 'aceptado': return { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-600', icon: CheckCircle2, label: 'Registrado' };
            case 'rechazado': return { bg: 'bg-red-500/10', border: 'border-red-500/20', text: 'text-red-600', icon: XCircle, label: 'Rechazado' };
            case 'caducado': return { bg: 'bg-slate-500/10', border: 'border-slate-500/20', text: 'text-slate-500', icon: Clock, label: 'Caducado' };
            default: return { bg: 'bg-slate-100', border: 'border-slate-200', text: 'text-slate-500', icon: Clock, label: status };
        }
    };
            ...
            <div className="relative z-10 flex-1 w-full max-w-5xl mx-auto px-4 py-4 md:py-6 space-y-4 font-sans selection:bg-indigo-500/30">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                        ...
                        <div className="flex p-0.5 bg-muted/50 border border-border rounded-xl w-full sm:w-fit self-end">
                            <button 
                                onClick={() => setActiveTab("registro")}
                                className={`flex-1 sm:flex-none px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all gap-1.5 flex items-center justify-center ${activeTab === "registro" ? "bg-background text-indigo-600 shadow-sm border border-border/50" : "text-muted-foreground hover:text-foreground"}`}
                            >
                                        ...
                                        >
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {req.status === 'pendiente' ? (
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <button
                                                                onClick={() => handleProcess(req.id, req.whatsapp, req.fullName)}
                                                                className="h-8 px-3 rounded-lg border border-blue-200 bg-blue-50 text-blue-600 text-[8px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all flex items-center gap-1.5"
                                                            >
                                                                <Send className="w-3 h-3" /> Re-enviar
                                                            </button>
                                                            <button
                                                                onClick={() => handleUpdateStatus(req.id, "aceptado")}
                                                                className="h-8 px-3 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-600 text-[8px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all flex items-center gap-1.5"
                                                            >
                                                        ...
                                                        <button
                                                            ...
                                                            className="h-8 px-3 rounded-lg bg-muted text-muted-foreground text-[8px] font-black uppercase tracking-widest hover:bg-border transition-all flex items-center gap-1.5"
                                                        >
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center text-muted-foreground border border-border hover:bg-red-500 hover:text-white hover:border-red-500 transition-all active:scale-90"
                                                    >
                                            ...
                                            {generatedLinks[req.id] && req.status === 'enviado' && (
                                                <motion.div
                                                    ...
                                                    className="mt-3 pt-3 border-t border-border/50 flex flex-col gap-2 relative z-10"
                                                >
                                                    <div className="flex flex-col gap-1.5">
                                                        <label className="text-[7px] font-black uppercase tracking-widest text-indigo-600 ml-1">Tactical Invite Link</label>
                                                        <div className="flex gap-2">
                                                            <div className="flex-1 bg-muted/50 border border-border rounded-lg px-3 py-1.5 text-[9px] font-mono overflow-hidden text-ellipsis whitespace-nowrap text-indigo-600 ">
                                    ...
                                    >
                                        <div className="flex flex-col gap-2.5 relative z-10">
                                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center border shadow-sm transition-transform group-hover:scale-105 ${msg.status === 'pendiente' ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-600' : 'bg-muted border-border text-muted-foreground'}`}>
                                                    ...
                                                    <div className="flex flex-col">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {msg.status === 'pendiente' && (
                                                                <span className="px-1.5 py-0.5 rounded-md border border-indigo-500/30 bg-indigo-500/10 text-indigo-600 text-[7px] font-black uppercase tracking-widest">Nuevo</span>
                                                            )}
                                                        </div>
                                                        <div className="flex items-center gap-3 mt-1">
                                                            <span className="flex items-center gap-1 text-indigo-600 font-black text-[8px] uppercase tracking-widest">
                                                ...
                                                <div className="flex items-center gap-2 ml-auto md:ml-0">
                                                    <button
                                                        ...
                                                        className={`h-7 px-3 rounded-lg border transition-all active:scale-95 text-[7px] font-black uppercase tracking-widest flex items-center gap-1.5 ${msg.status === 'leido' ? 'bg-muted text-muted-foreground border-border hover:text-foreground' : 'bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700'}`}
                                                    >
                                                        {msg.status === 'leido' ? <><EyeOff className="w-3 h-3" /> Pendiente</> : <><Eye className="w-3 h-3" /> Leído</>}
                                                    </button>
                                                    <button
                                                        onClick={() => handleDeleteMessage(msg.id)}
                                                        className="w-7 h-7 rounded-lg bg-muted flex items-center justify-center text-muted-foreground border border-border hover:bg-red-500 hover:text-white hover:border-red-500 transition-all active:scale-95"
                                                    >
                                                        <Trash2 className="w-3.5 h-3.5" />
                                                    </button>
                                                </div>
                                            ...
                                            <div className="p-2.5 bg-card/40 border border-border/50 rounded-lg space-y-1">
                                                <p className="text-[8px] font-black uppercase tracking-[0.2em] text-indigo-600/70 italic leading-none">{msg.subject}</p>
                                            ...
                                            <div className="flex justify-end pt-0.5">
                                                <a 
                                                    ...
                                                    className="h-7 px-4 bg-foreground text-background rounded-lg text-[7px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 hover:opacity-90 active:scale-95 shadow-sm"
                                                >
