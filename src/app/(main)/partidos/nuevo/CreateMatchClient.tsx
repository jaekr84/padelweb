...
export default function CreateMatchClient({ categories }: { categories: Category[] }) {
    ...
    const inputClasses = "w-full bg-muted/20 border border-border/60 rounded-xl px-4 py-2.5 text-xs font-black uppercase tracking-wider text-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:ring-1 focus:ring-azul-primary/45 focus:border-azul-primary transition-all";
    const labelClasses = "block text-[7.5px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-1.5 ml-0.5";
    ...
    return (
        <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 relative z-10">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-card border border-border/60 rounded-2xl p-6 shadow-sm space-y-6">
                    <div>
                        <h3 className="text-xs font-black uppercase tracking-wider text-foreground leading-none">Parámetros del Game</h3>
                        <p className="text-[10px] text-muted-foreground/60 font-bold mt-1 uppercase tracking-tight">Especificá la fecha, hora y ubicación física.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground/60" />
                                <input
                                    type="date"
                                    required
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                <Clock className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground/60" />
                                <input
                                    type="time"
                                    required
                                    className={`${inputClasses} pl-10 cursor-pointer [color-scheme:dark]`}
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground/60" />
                                <input
                                    type="text"
                                    required
                        ...
                        <div className="md:col-span-2">
                            <div className="relative w-full aspect-[21/9] rounded-xl overflow-hidden border border-border/60 bg-muted/20 group/map shadow-inner">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                <Trophy className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground/60" />
                                <select
                                    className={`${inputClasses} pl-10 appearance-none pr-8 cursor-pointer font-black text-foreground bg-muted/30`}
                                    value={formData.category}
                                    onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                                >
                                    <option value="Libre" className="bg-background text-foreground">Libre</option>
                                    {categories.map(cat => (
                                        <option key={cat.id} value={cat.name} className="bg-background text-foreground">{cat.name}</option>
                                    ))}
                                </select>
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                <Users className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground/60" />
                                <select
                                    className={`${inputClasses} pl-10 appearance-none pr-8 cursor-pointer font-black text-foreground bg-muted/30`}
                                    value={formData.gender}
                                    onChange={(e) => setFormData({ ...formData, gender: e.target.value })}
                                >
                                    <option value="mixto" className="bg-background text-foreground">Mixto</option>
                                    <option value="masculino" className="bg-background text-foreground">Masculino</option>
                                    <option value="femenino" className="bg-background text-foreground">Femenino</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-card border border-border/60 rounded-2xl p-6 shadow-sm space-y-6">
                    <div>
                        ...
                        <p className="text-[10px] text-muted-foreground/60 font-bold mt-1 uppercase tracking-tight">Gestioná el listado inicial de inscriptos.</p>
                    </div>
                    ...
                    <div className="space-y-4">
                                    ...
                                    <AnimatePresence>
                                        {searchResults.length > 0 && (
                                            <motion.div
                                                ...
                                                exit={{ opacity: 0, y: -5 }}
                                                className="absolute z-50 w-full mt-1.5 bg-popover border border-border rounded-xl shadow-2xl overflow-hidden max-h-48 overflow-y-auto"
                                            >
                                                {searchResults.map((user) => (
                                                    <button
                                                        ...
                                                        className="w-full p-2.5 flex items-center gap-2.5 hover:bg-primary/10 transition-colors text-left border-b border-border/30 last:border-0"
                                                    >
                                                        <div className="w-8 h-8 rounded-full overflow-hidden relative border border-border bg-muted shrink-0 flex items-center justify-center">
                                                            {user.imageUrl ? (
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        type="button"
                                        onClick={addGuest}
                                        className="bg-slate-900 border border-border/60 hover:bg-azul-primary text-white px-4 rounded-xl font-black uppercase tracking-widest text-[9px] transition-all shrink-0 h-9 flex items-center justify-center"
                                    >
                                        Sumar
                                    </button>
                                </div>
                        ...
                        <div className="space-y-3 pt-3 border-t border-border/40">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded bg-muted/40 text-muted-foreground tracking-widest">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center justify-between p-2.5 bg-red-500/5 rounded-xl border border-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.03)]">
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[8px] font-bold text-muted-foreground/60 uppercase tracking-widest leading-none">Creador del Match</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {selectedPlayers.map((player, idx) => (
                                    <motion.div
                                        ...
                                        key={idx}
                                        className="flex items-center justify-between p-2 bg-muted/20 border border-border/40 rounded-xl group"
                                    >
                                        <div className="flex items-center gap-2">
                                            <div className="w-7 h-7 rounded-full overflow-hidden relative border border-border bg-background flex items-center justify-center shrink-0">
                                                {player.imageUrl ? (
                ...
                <div className="lg:col-span-3 pt-4 border-t border-border/40">
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full h-11 bg-slate-950 hover:bg-azul-primary text-white border border-border/60 hover:border-azul-primary/40 rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg hover:shadow-azul-primary/20 active:scale-98 transition-all flex items-center justify-center gap-2"
                    >
                        {loading ? (
                            "PUBLICANDO MATCH..."
                        ) : (
